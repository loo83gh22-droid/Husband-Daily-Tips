-- Fix incorrectly awarded challenge_completed badges
...
-- Remove challenge_completed badges from users who haven't actually completed all 7 days
DELETE FROM user_badges
WHERE badge_id IN (
  SELECT b.id
  FROM badges b
  WHERE b.requirement_type = 'challenge_completed'
)
AND user_id IN (
...
-- by requiring completed_days = 7 in addition to completed = true for challenge_completed badges
